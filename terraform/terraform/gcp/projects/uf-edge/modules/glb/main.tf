...
resource "google_compute_url_map" "default" {
  ...
  path_matcher {
    ...
    dynamic "route_rules" {
      ...
      content {
        priority = index(keys(var.url_map), route_rules.key) + 1
        service  = google_compute_backend_service.path[route_rules.key].self_link
        match_rules {
          ignore_case  = true
...
resource "google_compute_backend_service" "path" {
  ...
  name                  = "glb-${local.name}-${replace(replace(each.key, "/", "-"), ".", "-")}"
  port_name             = "http"
  protocol              = "HTTP"
  timeout_sec           = 10
  load_balancing_scheme = "EXTERNAL_MANAGED"
