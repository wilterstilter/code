...
resource "google_service_account" "crowdstrike_sa" {
  account_id   = "crowdstrike-cspm-integration"
  display_name = "CrowdStrike CSPM Integration"
  description  = "Service account for CrowdStrike CSPM integration with GCP"
  project      = var.project_id
}

# Create service account key
resource "google_service_account_key" "crowdstrike_sa_key" {
  service_account_id = google_service_account.crowdstrike_sa.name
}
...
resource "google_secret_manager_secret" "secret_basic_sa" {
  secret_id = "crowdstrike-cspm-sa-secret"
...
resource "google_secret_manager_secret_version" "secret_basic_sa_version" {
  ...
  secret_data = base64decode(google_service_account_key.crowdstrike_sa_key.private_key)
}
