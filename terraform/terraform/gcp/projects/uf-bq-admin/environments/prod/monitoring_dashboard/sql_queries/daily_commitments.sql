...
commitments AS (
    ...
    FROM
        `region-us-south1`.INFORMATION_SCHEMA.CAPACITY_COMMITMENT_CHANGES_BY_PROJECT AS cccp
...
days AS (
    ...
    FROM (
        SELECT
            ...
            stop_date
        FROM results
    ), UNNEST(GENERATE_DATE_ARRAY(start_date, stop_date)) AS day
)

SELECT
    TIMESTAMP(day) AS date_d,
    LAST_VALUE(slot_cummulative IGNORE NULLS) OVER (
        ORDER BY day
    ) AS slots
FROM days
