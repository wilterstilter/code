...
resource "google_service_account_key" "looker_key" {
  ...
  service_account_id = google_service_account.looker_service_account[each.value.account_id].name
}

# Create service account key Id to secret manager
resource "google_secret_manager_secret" "looker_secret_key_id" {
  for_each  = local.generate_key_accounts
...
resource "google_secret_manager_secret_version" "looker_secret_key_version" {
  for_each    = local.generate_key_accounts
  secret      = google_secret_manager_secret.looker_secret_key_id[each.value.account_id].id
  secret_data = base64decode(google_service_account_key.looker_key[each.value.account_id].private_key)
}
...
resource "google_secret_manager_secret_iam_member" "looker_secret_access" {
  for_each  = local.generate_key_accounts
  secret_id = google_secret_manager_secret.looker_secret_key_id[each.value.account_id].id
