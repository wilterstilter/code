...
resource "google_artifact_registry_repository" "cvs-ptms" {
  project       = var.project_id
  location      = "us"
  repository_id = "cvs-ptms-docker"
  description   = "Uber Freight Docker registry to store PTMS Images"
  format        = "DOCKER"
...
resource "google_artifact_registry_repository_iam_binding" "cvs-ptms_viewer" {
  project    = google_artifact_registry_repository.cvs-ptms.project
  location   = google_artifact_registry_repository.cvs-ptms.location
  repository = google_artifact_registry_repository.cvs-ptms.name
  role       = "roles/artifactregistry.reader"
  members = concat(
    [
      local.all_users,
      "serviceAccount:ptms-cvs@uf-build-p.iam.gserviceaccount.com"
    ]
  )
}

# Read/Write access is given to all engineers (under Val)
resource "google_artifact_registry_repository_iam_binding" "cvs-ptms_writer" {
  project    = google_artifact_registry_repository.cvs-ptms.project
  location   = google_artifact_registry_repository.cvs-ptms.location
  repository = google_artifact_registry_repository.cvs-ptms.name
...
resource "google_artifact_registry_repository_iam_binding" "cvs-ptms_admin" {
  project    = google_artifact_registry_repository.cvs-ptms.project
  location   = google_artifact_registry_repository.cvs-ptms.location
  repository = google_artifact_registry_repository.cvs-ptms.name
