...
resource "google_secret_manager_secret_version" "secret_key_version" {
  for_each    = local.generate_key_accounts
  secret      = google_secret_manager_secret.secret_key_id[each.key].id
  secret_data = base64decode(google_service_account_key.key[each.key].private_key)
}

resource "google_secret_manager_secret_iam_member" "secret_access" {
  for_each  = local.generate_key_accounts
  secret_id = google_secret_manager_secret.secret_key_id[each.key].id
  role      = "roles/secretmanager.secretAccessor"
  member    = "group:${local.freight_mle_group}"
}
