...
inputs = {
  ...
  pool_id                    = "wif-dev"                                           # Name of WIF Pool Id
  pool_name                  = "wif-dev"                                           # The display name of the wif pool
  pool_description           = "Dev WIF pool for authentication of composer CI/CD" # The description of the wif pool and what it is used for
  pool_disabled              = false                                               # A boolean depicting whether the wif pool is disabled or not. Should always be false
  pool_provider_id           = "terraform-cloud-oidc-dev"                          # The unique identifier for the Workload Identity Pool Provider within the specified pool.
  pool_provider_display_name = "oidc_provider_for_dev"                             # A human-readable name for the Workload Identity Pool Provider.
  ...
  service_account_id           = "wif-storage-dev"                 # WIF service account name dedicated to dev
  service_account_display_name = "Dev WIF Storage Service Account" # WIF service account display name for dev
  project_id                   = include.gcp.locals.project_id
